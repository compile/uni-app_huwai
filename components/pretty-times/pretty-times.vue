				...
				<block v-for="(item,_index) in timeArr" :key="_index">
					<view class="item">
						...
						'active':item.time == timeQuanBegin || item.time == timeQuanEnd }"
						 :style="{color:item.time == timeQuanBegin || item.time == timeQuanEnd? selectedItemColor:'#333'}" @click="handleSelectQuantum(_index,item)">
							<text>{{item.time}}</text>
							<text class="all">{{item.disable?disableText:undisableText}}</text>
						</view>
					</view>
		...
		methods: {
			initOnload() {
				...
				this.timeArr.forEach((item, index) => {
					...
					console.log('cur_time:'+cur_time);//当前时间
					console.log('test');
					console.log(this.disableTimeSlot);
					
					this.disableTimeSlot.forEach((item2)=>{
						console.log('item.begin_time:'+item2.begin_time);
						console.log('item.end_time:'+item2.end_time);
						
						
						
						const begin_time = item2.begin_time;
						const end_time = item2.end_time;

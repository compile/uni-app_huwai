		...
		methods: {
			...
			async handleSubmit() {				if(!this.mobile) return uni.showToast({title: '请输入手机号',icon:'none',duration: 1500})				if(!this.code) return uni.showToast({title: '请输入验证码',icon:'none',duration: 1500})				if(!this.password) return uni.showToast({title: '请输入密码',icon:'none',duration: 1500})
				...
				if(!(/^1[3456789]\d{9}$/.test(this.mobile))){
					return uni.showToast({title: '手机号码不正确',icon:'none',duration: 1500})
				}
				console.log('');//这里检测 验证码是否正确
				// 逻辑:发送手机号.服务器存储手机号和 验证码. 然后在这里输入验证码. 比对. 如果是对的. 就通过注册 . 并且直接登陆.
				...
				const res = await this.$myRequest({url: '/api/user/register',method:'post',data:sendData});//获得验证码
				...
				console.log(sendData);
				const res = await this.$myRequest({url: '/api/user/verificationCode',method:'post',data:sendData});//获得验证码
				console.log('res.data:'+JSON.stringify(res.data));

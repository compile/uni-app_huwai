<template>
  <view class="waterfalls-box " :style="{height:height+'px'}" >
    ...
    <view
      ...
      class="waterfalls-list animation-bottom-top"
      :key="item[idKey]"
	  :data-index="index"
      :id="'waterfalls-list-id-'+item[idKey]"
      :ref="'waterfalls-list-id-'+item[idKey]"
      :style="{'--offset':offset + 'px','--cols':cols,top:allPositionArr[index].top||0,left:allPositionArr[index].left||0,'animationDelay': (index + 1)*0.1 + 's'}"
      @click="hrefstoreDetail"
    >
      <image
...
export default {
  props: {
    ...
    // 图片 src 的键名
    imageSrcKey: { type: String, default: "imgList" },
    // 列数
  ...
  methods: {
	...
	hrefstoreDetail(e){
		this.$emit("wapperlick",e.currentTarget.dataset.index)

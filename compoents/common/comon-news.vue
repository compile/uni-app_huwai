<template>
	<view class="common-list u-f-d animated zoomInUp fast">
		 ...
		 <view class="common-list-right">
			 ...
			 <view @tap="navToTopicDetailPage(item)">{{item.title}}</view>
			 <view @tap="navToTopicDetailPage(item)" class="u-f-ajc">
	         	 <!--图文-->
				 <image :src="item.titleimg" v-if="item.titleimg"></image>
				 <!--视频-->
				 ...
				 <view class="u-f-aj list-news" v-if="item.sharea">
					 <image :src="item.sharea.shareimg" mode="aspectFill"></image>
	...
	export default {
		...
		methods:{
			...
			navToTopicDetailPage(item){
				...
				if(item.type == 2){
						uni.navigateTo({
							url: `/pages/post/video-detail?id=${id}`
						})
				}else{
					uni.navigateTo({

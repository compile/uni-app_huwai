	...
	<view class="wrap">
		...
		<view class="head">
			...
			<view class="head-c">
				...
				<view class="count">
					...
					<u-button style="margin-right: 46rpx" class="mg-left-auth" :custom-style="btnStyle" @click="selectscenic">ifun</u-button>
				</view>
			</view>
		</view>
		<!-- 简介 -->
		<navigator class="member-wrap" :url="'/pages/topic/topic-user?id=' + topicId">
			<view class="member-wrap-head">
				<text class="notice-txt u-line-1">{{ topicInfo.description }}</text>
...
export default {
	...
	methods: {
		...
		onPlus() {
			// if (this.topicInfo.is_join) {   需要加入才能发帖吗？？
		...
		selectscenic(){//点击这里告诉app说我要去这里玩。让我选择套餐。下单就好。
			console.log('id:'+ this.topicId);
			uni.navigateTo({
				url:'/pages/product/grouplist'
			})
		},
		outTopic() {
			this.$H

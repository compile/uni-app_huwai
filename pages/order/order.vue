					...
					>
						...
						<view >
							<text class="right"></text>
							 <!-- <img style="height:100upx;width:100upx;" :src="item.scenicinfo.bg_image"> -->
							<text class="price">{{item.scenicinfo.description}}</text>
						</view>
						...
						display: flex;">行程号:{{item.order_sn}}</text>
										...
										<button class="action-btn" @click="cancelOrder(item)">取消预定</button>
						</view>
						<view class="action-box b-t" v-else-if="item.status == 1">
							<text class="left" style="font-size:20rpx;float:left;flex: 1;
			display: flex;">行程号:{{item.order_sn}}</text>
							<button class="action-btn recom">取消</button>
						...
						<view class="action-box b-t" v-else-if="item.status == 2">
										<text class="left" style="font-size:20rpx;float:left;flex: 1;
						display: flex;">行程号:{{item.order_sn}}</text>
										<button class="action-btn" @click="cancelOrder(item)">预约中</button>
										<button class="action-btn recom">取消</button>
						</view>
						<view class="action-box b-t" v-else-if="item.status == 3">
							<text class="left" style="font-size:20rpx;float:left;flex: 1;
							display: flex;">行程号:{{item.order_sn}}</text>
						...
						<view class="action-box b-t" v-else-if="item.status == 4">
							<text class="left" style="font-size:20rpx;float:left;flex: 1;
			...
			async	loadData(source){
				...
				setTimeout(()=>{
					...
					orderList.forEach((item, index) => { 
						this.goodslist.push(JSON.parse(orderList[index].goodslist));
						
						if(index == 'goodslist'){
							console.log('hhhhhhhhhhhhhhhhhhgoodslist');
						}
